# ダウンロード方法
GitHubのリリースページからインストーラーをダウンロードして、実行してください。

## 1. GitHubのリリースページにアクセス
まずは、[GitHubのリポジトリ](https://github.com/Ringoacid/image-cropper)から、リリースページにアクセスします。

![](Assets/ダウンロードスクショ/1_GitHubリポジトリ.webp)

## 2. インストーラーのダウンロード
リリースページから最新のインストーラーをダウンロードします。

![](Assets/ダウンロードスクショ/2_リリースページ.webp)

# インストール方法
インストーラーをダブルクリックして実行すると、セットアップウィザードが開始されます。以下の手順に従ってインストールを進めてください。

## 1. 言語の選択
インストール中に使用する言語を選択します。このドキュメントでは日本語を選択します。

![](Assets/インストールスクショ/1_言語の選択.webp)

## 2. 使用許諾契約書の同意
このアプリ（ImageCropper）は、MITライセンスのもとで配布されており、使用許諾契約書に同意する必要があります。内容を確認して、同意できる場合は「同意する」を選択してください。

![](Assets/インストールスクショ/2_使用許諾契約書の同意.webp)

## 3. デスクトップアイコンの作成
インストール後にデスクトップアイコンを作成するかどうかを選択します。必要に応じてチェックを入れてください。

![](Assets/インストールスクショ/3_デスクトップアイコン作成.webp)

## 4. インストールの準備完了
内容を確認して、インストールの準備ができたら「インストール」をクリックしてください。

![](Assets/インストールスクショ/4_インストール準備完了.webp)

## 5. インストール中の様子
圧縮されたファイルを展開して、必要なファイルをインストール先にコピーしています。

![](Assets/インストールスクショ/5_インストール中の様子.webp)

## 6. ThirdPartyNoticeについて
このアプリではNuGetパッケージを使用しています。帰属表示や、ライセンス全文などの情報は、インストール先の ThirdPartyNotices.md を参照してください。

![](Assets/インストールスクショ/6_ThirdPartyNoticeについて.webp)

## 7. セットアップ完了
インストールが完了しました。「完了」をクリックしてセットアップウィザードを終了してください。

![](Assets/インストールスクショ/7_セットアップ完了.webp)

